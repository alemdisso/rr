...
class Ruth_Collection_EditionMapper
{
    ...
    public function insert(Ruth_Collection_Edition $obj)
    {
    ...
    public function update(Ruth_Collection_Edition $obj)
    {
        if (!isset($this->identityMap[$obj])) {
            throw new Ruth_Collection_EditionMapperException('Object has no ID, cannot update.');
        }
        ...
        try {
            ...
            throw new Ruth_Collection_EditionException("sql failed");
        }
    ...
    public function findById($id)
    {
        ...
        $result = $query->fetch();
        if (empty($result)) {
            throw new Ruth_Collection_EditionMapperException(sprintf('There is no edition with id #%d.', $id));
        }
        $work = $result['work'];
        $obj = new Ruth_Collection_Edition();
    ...
    public function findByWork($work)
    {
        ...
        if (empty($result)) {
            throw new Ruth_Collection_EditionMapperException(sprintf('There is no edition with work #%s.', $work));
        }
        $id = $result['id'];
        ...
        if ($id > 0) {
            ...
            throw new Ruth_Collection_EditionMapperException(sprintf('The edition with id #%s has id=0?!?.', $work));
        }
    }


    public function delete(Ruth_Collection_Edition $obj)
    {
        if (!isset($this->identityMap[$obj])) {
            throw new Ruth_Collection_EditionMapperException('Object has no ID, cannot delete.');
        }
   ...
   public function getAllEditionsAlphabeticallyOrdered()
    ...
    private function setAttributeValue(Ruth_Collection_Edition $a, $fieldValue, $attributeEditor)
    {
