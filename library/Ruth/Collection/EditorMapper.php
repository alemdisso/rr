...
class Ruth_Collection_EditorMapper
{
    ...
    public function insert(Ruth_Collection_Editor $obj)
    {
    ...
    public function update(Ruth_Collection_Editor $obj)
    {
        if (!isset($this->identityMap[$obj])) {
            throw new Ruth_Collection_EditorMapperException('Object has no ID, cannot update.');
        }
        ...
        try {
            ...
            throw new Ruth_Collection_EditorException("sql failed");
        }
    ...
    public function findById($id)
    {
        ...
        if (empty($result)) {
            throw new Ruth_Collection_EditorMapperException(sprintf('There is no editor with id #%d.', $id));
        }
        $uri = $result['uri'];

        $obj = new Ruth_Collection_Editor();
    ...
    public function findByUri($uri)
    {
        ...
        if (empty($result)) {
            throw new Ruth_Collection_EditorMapperException(sprintf('There is no editor with uri #%s.', $uri));
        }
        $id = $result['id'];
        ...
        if ($id > 0) {
            ...
            throw new Ruth_Collection_EditorMapperException(sprintf('The editor with id #%s has id=0?!?.', $uri));
        }
    }


    public function delete(Ruth_Collection_Editor $obj)
    {
        if (!isset($this->identityMap[$obj])) {
            throw new Ruth_Collection_EditorMapperException('Object has no ID, cannot delete.');
        }
   ...
   public function getAllEditorsAlphabeticallyOrdered()
    ...
    private function setAttributeValue(Ruth_Collection_Editor $a, $fieldValue, $attributeName)
    {
