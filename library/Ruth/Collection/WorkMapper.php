...
class Ruth_Collection_WorkMapper
{
    ...
    public function insert(Ruth_Collection_Work $obj)
    {
    ...
    public function update(Ruth_Collection_Work $obj)
    {
        if (!isset($this->identityMap[$obj])) {
            throw new Ruth_Collection_WorkMapperException('Object has no ID, cannot update.');
        }
        ...
        try {
            ...
            throw new Ruth_Collection_WorkException("sql failed");
        }
    ...
    public function findById($id)
    {
        ...
        if (empty($result)) {
            throw new Ruth_Collection_WorkMapperException(sprintf('There is no work with id #%d.', $id));
        }
        $uri = $result['uri'];
    ...
    public function findByUri($uri)
    {
        ...
        if (empty($result)) {
            throw new Ruth_Collection_WorkMapperException(sprintf('There is no work with uri #%s.', $uri));
        }
        $id = $result['id'];
        ...
        if ($id > 0) {
            ...
            throw new Ruth_Collection_WorkMapperException(sprintf('The work with id #%s has id=0?!?.', $uri));
        }

    }


    public function delete(Ruth_Collection_Work $obj)
    {
        if (!isset($this->identityMap[$obj])) {
            throw new Ruth_Collection_WorkMapperException('Object has no ID, cannot delete.');
        }
    ...
    private function setAttributeValue(Ruth_Collection_Work $a, $fieldValue, $attributeName)
    {
